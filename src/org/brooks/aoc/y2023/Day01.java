...
public class Day01 {
    public static class Phase1 implements Solution<Long> {
        ...
        public Long solve(Stream<String> input) {
            return input.map((s) -> s.replaceAll("[^0-9]", "")).map((s) -> {
                if (s.isEmpty()) {
                    return "0";
                }
                if (s.length() == 1) {
    ...
    public static class Phase2 implements Solution<Long> {
        private final List<String> allValues = Arrays.asList("one", "1", "two", "2", "three", "3", "four", "4", "five", "5", "six", "6", "seven", "7", "eight", "8", "nine", "9");
        private final Map<String, String> keyVal = Map.of("one", "1", "two", "2", "three", "3", "four", "4", "five", "5", "six", "6", "seven", "7", "eight", "8", "nine", "9");

        @Override
        public Long solve(Stream<String> input) {
            return input.map((s) -> allValues.stream().map((v) -> new Phase2.IndexAndValue(s.indexOf(v), s.lastIndexOf(v), v)).filter((v) -> v.firstIndex >= 0).collect(Collectors.teeing(Collectors.minBy(Comparator.comparing(o -> o.firstIndex)), Collectors.maxBy(Comparator.comparing(o -> o.lastIndex)), (min, max) -> new Phase2.Result(min.map(o -> o.value).map(o -> keyVal.getOrDefault(o, o)).orElse("0"), max.map(o -> o.value).map(o -> keyVal.getOrDefault(o, o)).orElse("0"))))).map((o) -> o.firstValue + o.secondValue).mapToLong(Long::parseLong).sum();
        }
